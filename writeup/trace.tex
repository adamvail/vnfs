...
The path from the network to the guest is detailed in appendix ?. %TODO add reference to appendix
...
i%The code attempts to reuse memory efficiently, but even with this optimization, we'll see that it still spends the majority of its time managing memory.
...
We also note that when user-mode networking is essentially duplicating much of the work already done by the guest's network stack in order to rewrite the correct header fields to make the packet
